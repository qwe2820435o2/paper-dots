...
/** `id` doubles as the key under `editor.momentCard.tabs`. */
...
export default function MomentCardApp() {
    ...
    const activeLabel = activePanel ? t(`momentCard.tabs.${activePanel}`) : null;
                ...
                {TOOLS.map(({ id, icon: Icon }) => {
                    ...
                    return (
                        ...
                        >
                            ...
                            <span className="text-[10px] leading-none font-semibold">{t(`momentCard.tabs.${id}`)}</span>
                        </button>
                    );
                })}
            </div>
